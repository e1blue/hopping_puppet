...
static void callback(void *param, void *buff, size_t size){
	...
	if(this->loading > 0){
		if(buff != NULL){
			// ロード成功
			unsigned char *rawBuff = base64decode(buff, sizeof(struct dataField));
			memcpy(&this->data, rawBuff, sizeof(struct dataField));
			char path[256];
			sprintf(path, "img/field/%s.png", this->data.code);
...
struct dataField *dataFieldGet(int fieldKind, int *e3dIdImage){
	if(list == NULL){exit(1);}
	if(fieldKind < 0){return NULL;}

	// 直前のデータ確認
	if(previous != NULL && previous->fieldKind == fieldKind){
	...
	// ロード開始
	char path[256];
	sprintf(path, "data/field/%02d.dat", fieldKind);
	data->loading = 1;
	platformPluginDataLocal(data, path, callback);

	return dataFieldGet(fieldKind, e3dIdImage);
}
